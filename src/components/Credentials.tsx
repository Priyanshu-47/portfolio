          ...
          <div className="mt-5 space-y-5">
            {education.map((edu) => (
              <div key={edu.school} className="card p-6">
                <p className="inline-flex items-center gap-1.5 font-mono text-xs text-indigo-300">
                  <Calendar className="h-3.5 w-3.5" />
                  {edu.period}
                </p>
                <h4 className="mt-2.5 font-display text-base font-semibold text-white">{edu.school}</h4>
                {edu.degree ? <p className="mt-1 text-sm text-slate-400">{edu.degree}</p> : null}
          ...
          <div className="mt-5 space-y-5">
            {certifications.map((cert) => (
              <div key={cert.title} className="card card-hover flex items-center gap-4 p-5">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-indigo-300">
